...
start_gateway() {
  ...
  if [ "$(id -u)" -eq 0 ]; then
    /usr/local/sbin/hermes-create-manual-gateway-intent setup-start >/dev/null 2>&1 || true
  elif command -v sudo >/dev/null 2>&1; then
    sudo /usr/local/sbin/hermes-create-manual-gateway-intent setup-start >/dev/null 2>&1 || true
  fi
  tmux has-session -t "$TMUX_SESSION" 2>/dev/null && HERMES_ALLOW_GATEWAY_STOP=manual-intent tmux kill-session -t "$TMUX_SESSION" || true
  HERMES_ALLOW_GATEWAY_STOP=manual-intent hermes gateway stop >/dev/null 2>&1 || true
...
print_run_command() {
  local self_wsl
  self_wsl="$(readlink -f "$0" 2>/dev/null || printf '%s' "$0")"
  local self_win="${self_wsl}"
  local startup_win="${self_wsl%/*}/enable-windows-startup.ps1"
  case "$self_wsl" in
    /mnt/?/*)
      local drive="${self_wsl#/mnt/}"
      drive="${drive%%/*}"
      local rest="${self_wsl#/mnt/$drive/}"
      self_win="$(printf '%s:\\%s' "$(printf '%s' "$drive" | tr '[:lower:]' '[:upper:]')" "$(printf '%s' "$rest" | tr '/' '\\')")"
      rest="${startup_win#/mnt/$drive/}"
      startup_win="$(printf '%s:\\%s' "$(printf '%s' "$drive" | tr '[:lower:]' '[:upper:]')" "$(printf '%s' "$rest" | tr '/' '\\')")"
      ;;
  esac
  cat <<EOF

Run this setup now, after Windows reboot, or after reinstalling Ubuntu WSL from PowerShell:
C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -NoLogo -ExecutionPolicy Bypass -Command "rws; wsl -d ubuntu -- bash $self_wsl"

Enable it at Windows logon and run it immediately:
C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "$startup_win" -RunNow
EOF
}
