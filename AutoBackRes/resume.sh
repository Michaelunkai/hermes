...
HERMES_AGENT_ROOT="${HOME_DIR}/.hermes/hermes-agent"
HERMES_CA_BUNDLE="${HERMES_CA_BUNDLE:-}"
...
HERMES_PROGRESS_WATCHDOG_SOURCE="/mnt/f/study/AI_ML/AI_and_Machine_Learning/Artificial_Intelligence/hermes/AutoBackRes/telegram_fleet_progress_watchdog.py"
HERMES_PROGRESS_WATCHDOG_ENSURE_SOURCE="/mnt/f/study/AI_ML/AI_and_Machine_Learning/Artificial_Intelligence/hermes/AutoBackRes/ensure_telegram_progress_watchdog.sh"
...
discover_homes() {
  ...
  for h in \
    ...
    "${HOME_DIR}/.hermes-michahermes5bot"; do
    [ -d "$h" ] || continue
...
def preserve_conflicting_path(path, reason):
    preserved_dir = main_home / "restore-preserved-symlinks"
    preserved_dir.mkdir(parents=True, exist_ok=True)
    preserved = preserved_dir / f"{path.parent.name}.{path.name}.preserved"
    index = 1
    while preserved.exists() or preserved.is_symlink():
        preserved = preserved_dir / f"{path.parent.name}.{path.name}.{index}.preserved"
        index += 1
    target = None
    if path.is_symlink():
        target = os.readlink(path)
    path.rename(preserved)
    details = [f"original={path}", f"preserved={preserved}", f"reason={reason}"]
    if target is not None:
        details.append(f"symlink_target={target}")
    (preserved_dir / f"{preserved.name}.txt").write_text("\n".join(details) + "\n", encoding="utf-8")
    print(f"REQUIRED_HOME_PRIVATE_PATH_PRESERVED={path}:{preserved}:{reason}")

def ensure_private_dir(home, name):
    item = home / name
    if item.exists() and item.is_dir():
        return
    if item.exists() or item.is_symlink():
        preserve_conflicting_path(item, "not_a_live_directory")
...
def write_env(home, token):
    ...
    home_channel = (
        env_value(home / ".env", "TELEGRAM_HOME_CHANNEL")
        or env_value(main_home / ".env", "TELEGRAM_HOME_CHANNEL")
        or test_chat
        or allowed.split(",", 1)[0].strip()
        or "716239770"
    )
    lines = [
        f"TELEGRAM_BOT_TOKEN={token}",
        f"TELEGRAM_ALLOWED_USERS={allowed}",
...
for item in required:
    ...
    if not token:
        ...
        continue
    for name in private_dirs:
        ensure_private_dir(target, name)
...
resolve_hermes_ca_bundle() {
  local candidate
  if [ -n "${HERMES_CA_BUNDLE:-}" ] && [ -f "$HERMES_CA_BUNDLE" ]; then
    printf '%s\n' "$HERMES_CA_BUNDLE"
    return 0
  fi
  if [ -x "${HERMES_AGENT_ROOT}/venv/bin/python" ]; then
    candidate="$("${HERMES_AGENT_ROOT}/venv/bin/python" - <<'PY' 2>/dev/null || true
try:
    import certifi
    print(certifi.where())
except Exception:
...
sync_progress_watchdog_files() {
  local target_script="${HOME_DIR}/.hermes/scripts/telegram_fleet_progress_watchdog.py"
  local target_ensure="${HOME_DIR}/.hermes/scripts/ensure_telegram_progress_watchdog.sh"
  local stamp backup
  mkdir -p "${HOME_DIR}/.hermes/scripts"
  chown "$USER_NAME:$USER_NAME" "${HOME_DIR}/.hermes/scripts" 2>/dev/null || true

  for pair in \
    "$HERMES_PROGRESS_WATCHDOG_SOURCE:$target_script" \
    "$HERMES_PROGRESS_WATCHDOG_ENSURE_SOURCE:$target_ensure"; do
    local src="${pair%%:*}"
    local dst="${pair#*:}"
    if [ ! -f "$src" ]; then
      log "WARNING: progress watchdog source missing: $src"
      continue
    fi
    ...
    if [ -f "$dst" ]; then
      ...
      backup="${dst}.bak-before-zero-token-${stamp}"
      cp -p "$dst" "$backup" 2>/dev/null || true
      chown "$USER_NAME:$USER_NAME" "$backup" 2>/dev/null || true
    fi
    cp "$src" "$dst"
    chmod 0755 "$dst" 2>/dev/null || true
    chown "$USER_NAME:$USER_NAME" "$dst" 2>/dev/null || true
    log "HERMES_PROGRESS_WATCHDOG_FILE_SYNCED=$dst"
  done
}
...
if heartbeat_at:
    try:
        ...
        if age > max_age:
            print(f"RUNTIME_WORKING=heartbeat_stale_no_reconnect:{int(age)}")
    except Exception as exc:
...
start_gateway_home() {
  ...
  mark_loaded_config_home "$h" || true
  local ca_bundle
  ca_bundle="$(resolve_hermes_ca_bundle || true)"
  if [ -z "$ca_bundle" ]; then
    log "ERROR: [$h] no usable CA certificate bundle found for Hermes gateway startup"
    return 22
  fi
  as_ubuntu tmux new-session -d -s "$session" \
    "cd '$HOME_DIR' && export $HERMES_RUNTIME_EXPORTS HERMES_HOME='$h' HERMES_AGENT_TIMEOUT=0 HERMES_AGENT_TIMEOUT_WARNING=0 HERMES_STALE_LOCK_RECOVERY_SECONDS=0 HERMES_MAX_ITERATIONS=1000 HERMES_AUTO_CONTINUE_FRESHNESS=0 HERMES_NO_INTERACTIVE_DEP_PROMPTS=1 HERMES_DISABLE_DEP_AUTOINSTALL=1 PYTHONUNBUFFERED=1 PYTHONPATH='$HERMES_AGENT_ROOT' SSL_CERT_FILE='$ca_bundle' REQUESTS_CA_BUNDLE='$ca_bundle' PATH='$HERMES_PATH' && exec '$HERMES_BIN' gateway run >>'$run_log' 2>&1" 9>&-
  rm -f "$repair_restart_marker" 2>/dev/null || true
  local max_wait poll_sleep
  max_wait="${HERMES_TRAY_GATEWAY_READY_TIMEOUT_SECONDS:-45}"
...
if [ -f "${HOME_DIR}/.hermes/scripts/ensure_uninterrupted_execution_contract.py" ]; then
  if as_ubuntu "${HOME_DIR}/.hermes/hermes-agent/venv/bin/python" "${HOME_DIR}/.hermes/scripts/ensure_uninterrupted_execution_contract.py" >"$RESUME_TMP_DIR/uninterrupted-execution-contract.out" 2>&1; then
    while IFS= read -r line; do log "$line"; done <"$RESUME_TMP_DIR/uninterrupted-execution-contract.out"
    log "HERMES_UNINTERRUPTED_EXECUTION_CONTRACT_OK=1"
  else
    while IFS= read -r line; do log "$line"; done <"$RESUME_TMP_DIR/uninterrupted-execution-contract.out"
    log "WARNING: uninterrupted execution contract validation failed; continuing restore resume so Telegram gateways still start"
  fi
fi
install_hermes_command_guard
