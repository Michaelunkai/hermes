...
discover_homes() {
  ...
  done < <(required_fleet_homes) | awk '!seen[$0]++'
}
...
if gateway_state == "startup_failed":
    print("READY_FAIL=gateway_startup_failed")
    raise SystemExit(2)
...
if state_pid != pid or gateway_state != "running" or telegram_state != "connected":
    ...
    if process_age < startup_grace:
        ...
        raise SystemExit(0)
    if state_pid == pid and gateway_state == "running" and telegram_state != "connected":
        print(f"READY_FAIL=telegram_state_not_connected:{telegram_state or 'unknown'}")
        raise SystemExit(2)
...
if mode == "polling":
    ...
    max_age = int(env.get("HERMES_TRAY_POLLING_HEARTBEAT_MAX_AGE_SECONDS", "90"))
    hard_fail_age = int(env.get("HERMES_TRAY_POLLING_HEARTBEAT_HARD_FAIL_SECONDS", "300"))
    process_age = int(env.get("HERMES_PROCESS_AGE_SECONDS", "999999") or "999999")
    startup_grace = int(env.get("HERMES_TRAY_STARTUP_GRACE_SECONDS", "120") or "120")
    print(f"READY_POLLING_HEARTBEAT_OK=age_seconds:{int(age)} max_seconds:{max_age}")
    if age > max_age:
        if active_agents > 0:
            print(f"READY_WORKING=telegram_polling_heartbeat_stale_active_work_preserved:{int(age)}")
            raise SystemExit(0)
        if process_age >= startup_grace and age >= hard_fail_age:
            print(f"READY_FAIL=telegram_polling_heartbeat_stale:{int(age)}")
            raise SystemExit(2)
        print(f"READY_WORKING=telegram_polling_heartbeat_stale_no_reconnect:{int(age)}")
        raise SystemExit(0)
elif mode == "unknown":
...
PY
  ...
  if [ -z "$home_channel" ] && [ -n "${TELEGRAM_ALLOWED_USERS:-}" ]; then
    home_channel="${TELEGRAM_ALLOWED_USERS%%,*}"
    if [ -n "$home_channel" ] && [ -w "$h/.env" ] && ! grep -q '^TELEGRAM_HOME_CHANNEL=' "$h/.env" 2>/dev/null; then
      printf 'TELEGRAM_HOME_CHANNEL=%s\n' "$home_channel" >> "$h/.env"
      chmod 600 "$h/.env" 2>/dev/null || true
      echo "READY_TELEGRAM_HOME_CHANNEL_REPAIRED=$h:$home_channel"
    fi
  fi
...
checks = {
    'telegram_realtime_progress_contract_missing': (
        'progress_mode = (' in run_text
        and 'tool_progress_enabled = progress_mode not in {"off", "log"}' in run_text
    ),
    'telegram_progress_human_formatter_missing': (
        'from agent.display import get_tool_emoji' in run_text
        and 'emoji = get_tool_emoji(tool_name' in run_text
    ),
    'gateway_progress_callback_missing': 'def progress_callback(' in run_text,
    ...
    'telegram_progress_config_driven_missing': (
        'progress_mode = (' in run_text
        and 'tool_progress_enabled = progress_mode not in {"off", "log"}' in run_text
    ),
}
...
watchdog_required = {
    'telegram_progress_watchdog_not_low_noise_mode': 'low_noise_edit_zero_token_v1' in watchdog_text and 'PROGRESS_LOW_NOISE_' in watchdog_text,
    'telegram_progress_watchdog_not_edit_in_place': 'editMessageText' in watchdog_text and 'sendMessage' in watchdog_text,
    'telegram_progress_watchdog_uses_model_or_account_api': all(x not in watchdog_text.lower() for x in ('account_usage', 'fetch_account_usage', 'openai', 'codex')),
    'telegram_progress_watchdog_state_not_preserved': all(x not in watchdog_text for x in ('.pop(', '.unlink(', 'rmtree(', 'remove(', 'truncate(')),
    'telegram_progress_watchdog_has_spam_phrases': all(x not in watchdog_text for x in ('I will keep sending', 'Still running:', '[x] Running tool', 'Current step:')),
}
for name, ok in watchdog_required.items():
    if not ok:
        print('READY_FAIL=' + name)
        raise SystemExit(66)
if not ensure.exists() or 'low_noise_edit_zero_token_v1' not in ensure.read_text(encoding='utf-8'):
    print('READY_FAIL=telegram_progress_watchdog_ensure_not_low_noise')
    raise SystemExit(66)
live = []
...
if not live:
    ...
    raise SystemExit(66)
print("READY_PROGRESS_ZERO_TOKEN_LOW_NOISE_OK=1")
...
run_text = (repo / 'gateway' / 'run.py').read_text(encoding='utf-8')
required_runtime = {
    'restart_marker_persistence_missing': 'self.session_store.mark_resume_pending(' in run_text,
    'fresh_interruption_recovery_missing': (
        'def _is_fresh_gateway_interruption(' in run_text
        and 'def _last_transcript_timestamp(' in run_text
    ),
    'automatic_resume_scheduler_missing': (
        'def _schedule_resume_pending_sessions(' in run_text
        and 'self._schedule_resume_pending_sessions()' in run_text
    ),
    'tool_tail_recovery_missing': 'elif _has_fresh_tool_tail:' in run_text,
    'auto_resume_zero_freshness_contract_missing': (
        'if window <= 0:\n        return True' in run_text
        and 'return current - timestamp <= window' in run_text
    ),
...
homes.extend(
    ...
    if (p / 'config.yaml').exists() and (p / '.env').exists()
)
checked = 0
for home in homes:
    if home.name != '.hermes':
...
if "TELEGRAM_REALTIME_PROGRESS" not in run_text:
    print("READY_WARN=telegram_realtime_progress_contract_missing")
if "Working on your request" not in run_text or "_progress_human_status" not in run_text:
    print("READY_WARN=telegram_progress_human_formatter_missing")
...
if 'elif qcmd.get("type") == "prompt"' not in run_text or 'Fall through to normal agent handling' not in run_text:
    print("READY_WARN=custom_slash_realtime_progress_not_wired")
if "source.platform == Platform.TELEGRAM:\n            tool_progress_enabled = False" in run_text:
    print("READY_WARN=telegram_progress_still_forced_off_in_gateway")
if "Telegram progress is config-driven" not in run_text:
    print("READY_WARN=telegram_progress_config_driven_missing")
...
check_hermes_agent_runtime() {
  ...
  if ! sudo -H -u "$USER_NAME" env HERMES_HOME="${HOME_DIR}/.hermes" PYTHONPATH="$root" "$root/venv/bin/python" - <<'PY' >/dev/null 2>&1
import gateway.run, gateway.status, hermes_cli.main
